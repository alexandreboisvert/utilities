...
apt install bash-completion bzip2 cifs-utils curl dos2unix ethtool exfat-fuse exfat-utils gawk gdebi-core git gzip html2text htop iotop iperf jq locate lsof mc oping p7zip p7zip-full p7zip-rar parted pv rsync shellcheck tar tcpdump telnet tmux tree unzip util-linux vim visidata wget
